...
void draw_text_area(WINDOW *win) {
        ...
        if (doc.text[i] == '\\' && i + 1 < text_len) {
            switch (doc.text[i+1]) {
                ...
                case 'u': // Underline toggle
                    ...
                    continue;
                case 'c': // Color change
                    if (i + 2 < text_len && isdigit(doc.text[i + 2])) {
    ...
    int lim = doc.cursor_pos;
    if (lim < 0) {
        lim = 0;
    }
    if ((size_t)lim > text_len) {
        lim = (int)text_len;
    }
    ...
    for (int i = 0; i < lim; i++) {
        if (doc.text[i] == '\n') {
            cursor_y++;
            cursor_x = 1;
        } else if (doc.text[i] == '\\' && (size_t)i + 1 < text_len) {
            // Skip formatting codes
            if (strchr("biucs", doc.text[i + 1])) {
                i++;
                if (strchr("cs", doc.text[i]) != NULL && (size_t)i + 1 < text_len) {
                    i++;
                }
            } else {
                cursor_x++;
...
void process_key(int ch) {
    ...
    size_t len = strlen(doc.text);

    if (ch == KEY_BACKSPACE || ch == 127) {
        if (doc.cursor_pos > 0) {
            memmove(&doc.text[doc.cursor_pos - 1], &doc.text[doc.cursor_pos],
                    len - (size_t)doc.cursor_pos + 1);
            doc.cursor_pos--;
        }
    } else if (ch == KEY_DC) { // Delete key
        if ((size_t)doc.cursor_pos < len) {
            memmove(&doc.text[doc.cursor_pos], &doc.text[doc.cursor_pos + 1], len - (size_t)doc.cursor_pos);
        }
    } else if (ch == KEY_LEFT) {
        ...
        // Insert newline
        memmove(&doc.text[doc.cursor_pos + 1], &doc.text[doc.cursor_pos], len - (size_t)doc.cursor_pos + 1);
        ...
        size_t curlen = strlen(doc.text);
        if (curlen < (size_t)(MAX_TEXT - 1)) {
            memmove(&doc.text[doc.cursor_pos + 1], &doc.text[doc.cursor_pos],
                    curlen - (size_t)doc.cursor_pos + 1);
...
void apply_formatting(int ch) {
    ...
    if (format_code[0] && strlen(doc.text) + strlen(format_code) < MAX_TEXT) {
        ...
        size_t flen = strlen(format_code);
        for (size_t i = 0; i < flen; i++) {
            doc.text[doc.cursor_pos + (int)i] = format_code[i];
        }

        doc.cursor_pos += (int)flen;
    }
}
